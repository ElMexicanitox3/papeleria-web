...
@section('content')
    ...
    @section('titleCard', 'Editar Marca - ' . $brand->name)

    @section('contentCard')
        <form action="{{route("brands.update", $brand->uuid)}}" method="post">
            ...
            <div class="p-4 grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 gap-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="name" name="name" class="w-full border-2 border-black-300 rounded-md p-2" value="{{old('name', $brand->name)}}">
                    @error('name')
            ...
            <div class="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4">
                ...
                <a href="{{ route('brands.home') }}" class="px-4 py-2 text-white bg-red-500 rounded-md hover:bg-red-600 text-center">Cancelar</a>
